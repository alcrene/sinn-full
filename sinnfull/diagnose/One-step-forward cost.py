...
#
#   :::{math}
#   :label: eq:one-step-forward-psim
#   \begin{gathered}\hat{p}_{\text{sim}}(\bar{x}^m_l \leq X_{t+1}^m < \bar{x}^m_{l+1} | X_t=\{x_t\}) \,.\end{gathered}
#   :::
#   
...
#
# For [*psim*](eq:one-step-forward-psim), we can in fact do better, and use a kernel density estimator instead of a histogram. This produces a smooth distribution, which is easier to visually compare to the histograms of [*pobj*](eq:one-step-forward-pobj) when the two distributions are plotted over each other. If we see any mismatch between the distributions, fits are unlikely to produce meaningful results.
#
# The manner in which we calculate the distributions [*psim*](eq:one-step-forward-psim) and [*pobj*](eq:one-step-forward-pobj) is fully described in [the implementation notebook](../diagnostics/one_step_cost.ipynb).
...
# %% [markdown]
# Having calculated the estimates of $P(X_{t+1} | X_t)$ based on the simulation [*psim*](eq:one-step-forward-psim) and the likelihood (objective function) [*pobj*](eq:one-step-forward-pobj), to see if they match. Note that if we are using a true likelihood, these really should match exactly; errors on secondary terms in either the simulation or likelihood equations may cause only slight variations.
#
